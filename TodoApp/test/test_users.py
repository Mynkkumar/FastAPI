...
def test_return_user(test_user):
    ...
    assert response.json()["username"] == "codingwithmayanktest"
    assert response.json()["email"] == "codingwithmayanktest@email.com"
    assert response.json()["first_name"] == "Mayank"
    assert response.json()["last_name"] == "Kumar"
    assert response.json()["role"] == "admin"
    assert response.json()["phone_number"] == "7667278909"
